 #!/bin/sh
  ...
  ##判断prod是否有ingress，如果没有就创建。
    kubectl -n $NS --kubeconfig=/etc/deploy/kube2prod get deployment|egrep -i $NEW_CI_PROJECT_NAMESPACE-$CI_PROJECT_NAME
    if [ $? -eq 0 ]; then
  ...
  ##canary环境发版
    ...
    kubectl -n $CI_COMMIT_BRANCH --kubeconfig=/etc/deploy/kube2canary get deployment|egrep -i $NEW_CI_PROJECT_NAMESPACE-$CI_PROJECT_NAME
    if [ $? -eq 0 ]; then
...
##非canary/prod发版   
    ...
    kubectl -n $CI_COMMIT_BRANCH --kubeconfig=/etc/deploy/config get deployment|egrep -i $NEW_CI_PROJECT_NAMESPACE-$CI_PROJECT_NAME
	  if [ $? -eq 0 ]; then   
