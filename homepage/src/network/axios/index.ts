...
//优先采用 RFC 5897  让与 url 直接通过 a 标签的下载的结果相同
...
class MyAxios {
  ...
  private initInterceptors() {
    ...
    this.axiosInstance.interceptors.request.use(
      (config) => {
        //headers 的 access-token 部分在请求拦截中加入
        ...
        console.log(`本次请求的 config 信息：`, config);
        return config;
      },
      (error) => {
        console.log(`axios 请求拦截部分报错，错误信息 error`, error);
        return Promise.reject(error);
      },
    );

    //响应拦截  从接口响应的数据在这里处理 例如解密等  时间发生在 then catch 前
    this.axiosInstance.interceptors.response.use(
      (response) => {
        // resBaseInfo 针对接口返回有基本格式的情况下 如上面导入的 resBaseInfo 基本请求返回体 基本返回体由 rsCode rsCause 和 data 构成
        const { data } = response;
        console.log('data', data);
        if (data.rsCode !== 0) {
          alert(`${data.rsCause}`);
          return Promise.reject(data.data); //假设后台的错误信息放在了 data 中  这里根据情况修改
        }
        if (data instanceof Blob) {
          //兼容一下下方的文件下载处理
          return response;
        } else {
          return data.data; //因为下方封装默认泛型默认定义到了 response 下的 data 下的 resBaseInfo 下的 data
        }
      },
      (error: AxiosError) => {
        console.log('axios 响应拦截部分发生错误，错误信息为', error);

        //需要对错误进行提示？
        //以下 Message 是 ElementUI 库的全局提示组件 当然我们可以更改
        //若 ElementUI 需要在头部引入   import { Message } from 'element-ui';
        /*    if(error?.response){
  ...
  upload<T>(data: Upload): Promise<T> {
    ...
    return this.axiosInstance.post(url, formData, {
      ...
      onUploadProgress,
      signal: controller ? controller.signal : undefined, //用于文件上传可以取消  只需在外部调用 controller.abort() 即可。参考//https://juejin.cn/post/6954919023205154824
    });
  }

  async uploadStream<T>(data: UploadStream): Promise<T> {
    const { url, file, controller, onUploadProgress } = data;
    /** generateSHA 生成文件 SHA256 hash  用于标识文件唯一性 往往会用上 这里会用到 crypto-js 库 **/
    ...
    return new Promise((resolve, reject) => {
      this.axiosInstance
        .get<Blob>(url, {
          ...
          responseType: 'blob',
          onDownloadProgress,
          signal: controller ? controller.signal : undefined, //用于文件下载可以取消  只需在外部调用 controller.abort() 即可。参考//https://juejin.cn/post/6954919023205154824 以及 https://axios-http.com/zh/docs/cancellation
        })
        .then((res) => {
