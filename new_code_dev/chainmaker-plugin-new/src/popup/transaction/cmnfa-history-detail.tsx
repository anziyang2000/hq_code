...
export function NfaDetailPage() {
  ...
  // const txType = txInfo.from === currentAccount.address ? '转出' : '转入';
  const txType = contractType ? contractType : txInfo.fromAddress === currentAccount?.address ? '转出' : '转入';
  // console.log('yyyyyyyyyyyyyyy---currentAccount:', currentAccount);
  // console.log('yyyyyyyyyyyyyyy---txType:', txType);
  // console.log('txInfo:', txInfo);
  // console.log('具体详情页面---nonce:', nonce);
  // console.log('具体详情页面---blockNumber:', blockNumber);
  // console.log('具体详情页面---gasLimit:', gasLimit);
  // console.log('具体详情页面---contractName:', contractName);
  ...
  const detailInfo: Array<TextInfoItem | null> = [
    {
      ...
      value: (
        <a
          href={selectedChain?.browserLink ? `${selectedChain.browserLink.replace(/\/$/, '')}/tx/${txId}` : undefined}
          target="_blank"
          rel="noopener noreferrer"
          style={{ color: '#3a7bef', textDecoration: 'none', cursor: 'pointer' }}
          onClick={(e) => {
            if (!selectedChain?.browserLink) {
              e.preventDefault(); // 阻止跳转
              message.warning({ content: '当前节点未配置区块链浏览器地址，无法查看。' });
            }
          }}
        >
          在区块链浏览器中查看
