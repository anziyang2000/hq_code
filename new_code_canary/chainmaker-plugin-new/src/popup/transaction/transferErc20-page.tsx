...
function TransferErc20Page() {
  ...
  console.log('3. erc20转账页面---currentAccount:', currentAccount);
  ...
  return (
    <DetailPage title="代币转账" backUrl="/">
      <div className="signature">
        <Form layout="vertical">
          {/* 显示当前NFT信息 */}
          <Form.Item>
            <div>
              <p>当前持有数量: {ethers.utils.formatEther(balance)}</p>
            </div>
          </Form.Item>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Form.Item
                ...
                message={fieldState.error?.message}
                extra={`当前持有数量：${ethers.utils.formatEther(balance)}`}
              >
                <Input
                  {...field}
                  placeholder="请输入转账数量"
...
export default TransferErc20Page;