...
class query extends Contract {
    ...
    async QueryProjectIdExists(ctx, targetProjectId) {
        try {
            const queryString = JSON.stringify({
                selector: {}, // 空的 selector 表示查询所有记录
                sort: [{ project_id: 'asc' }] // 按 project_id 升序排序
            });
            ...
            // 检查是否存在与 targetProjectId 匹配的 project_id
            const exists = assets.some(asset => asset.Record.project_id === targetProjectId);

            return exists; // 返回 true 或 false
        } catch (error) {
            throw this.ErrorObj.createError(
                '',
                'QueryProjectIdExists: Failed to query project ID existence'
            );
        }
    }
    ...
    //         const queryString = JSON.stringify({
    //             selector: {
    //                 'instrument.project_id': targetProjectId
    //             },
    //             sort: [{ 'instrument.project_id': 'asc' }] // 按 instrument.project_id 排序
    //         });
    ...
    //         // 检查是否存在匹配的 project_id
    //         const exists = assets.some(asset => asset.Record.instrument.project_id === targetProjectId);
