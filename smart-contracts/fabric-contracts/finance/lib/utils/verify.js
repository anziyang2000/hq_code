...
class verify extends Contract {
    ...
    // 递归遍历对象，按ASCII码升序或降序排序
    // @param {Object} obj - 要排序的对象
    // @param {boolean} isSort - 排序标志，true为升序，false为降序
    // @returns {Object} - 排序后的对象
    async sortASCII(obj, isSort = true) {
        // 处理非对象或空对象的情况
        ...
        const sortObject = (o) => {
            ...
            // 如果是数组，保持数组结构，递归处理每个元素
            if (Array.isArray(o)) {
                return o.map(item => sortObject(item));
            }

            // 处理对象的键排序
            let keys = Object.keys(o);
            let sortedKeys = isSort ? keys.sort() : keys.sort().reverse();
            let sortedObj = {};

            for (let key of sortedKeys) {
                sortedObj[key] = sortObject(o[key]); // 递归处理嵌套的对象
            }
